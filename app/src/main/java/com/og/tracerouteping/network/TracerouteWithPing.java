...
import edu.purdue.combinekarttruck.BasicLoggingActivity;
 ...
 * @author Andrew Balmos (tweaks and bugfixes)
 * 
 */
public class TracerouteWithPing {
	...
	public TracerouteWithPing(BasicLoggingActivity context) {
	...
	public void executeTraceroute(String url, int maxTtl) {
		...
		Log.d(tag, "Started traceroute to " + this.urlToPing);
	...
	private class TimeOutAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			...
			runnableTimeout = new Runnable() {
				...
				public void run() {
					if (task != null) {
						Log.d(tag, ttlTask + " task.isFinished() " + finishedTasks + " " + (ttlTask == finishedTasks));
			...
			if (hasConnectivity()) {
				try {
					...
					if (res.contains(UNREACHABLE_PING) && !res.contains(EXCEED_PING)) {
						...
						trace = new TracerouteContainer(timestamp, "", ip, elapsedTime, false);
					} else {
						// Create the TracerouteContainer object when succeed
						trace = new TracerouteContainer(timestamp, "", ip, ttl == maxTtl ? Float.parseFloat(parseTimeFromPing(res))
								: elapsedTime, true);
					}

					// Get the host name from ip (unix ping do not support hostname resolving)
                    ...
                    if (!ip.equals(ipToPing) || ttl == maxTtl) {
                        context.tracerouteAddTrace(trace);
                    }

                    if (ttl == maxTtl) {
                        if (ip.equals(ipToPing)) {
                            Log.d(tag, "Completed (true) traceroute to " + urlToPing);
                            context.tracerouteComplete(true);
                        } else {
                            Log.d(tag, "Completed (false) traceroute to " + urlToPing);
                            context.tracerouteComplete(false);
                        }
                    }
